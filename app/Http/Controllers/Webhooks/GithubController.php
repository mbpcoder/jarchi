...
class GithubController extends WebhookBaseController
{
    public function handle(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $parser = new Github(json_decode($request->getContent(), false));
            $message = $parser->parseMessage();

            if ($message !== '') {
                $target = $target ?: $this->getDefaultTarget('github', $channel);
                ...
                $notification = new GithubNotification($message, $channel, $metadata);
