...
class GitlabController extends WebhookBaseController
{
    public function handle(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $parser = new Gitlab(json_decode($request->getContent(), false));
            $message = $parser->parseMessage();

            if ($message !== '') {
                $target = $target ?: $this->getDefaultTarget('gitlab', $channel);
                ...
                $notification = new GitlabNotification($message, $channel, $metadata);
